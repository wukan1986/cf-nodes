...
`订阅器`、`订阅转换后端`无要求。
1. `Pages`部署，文件夹中含`home.html/link.html/_worker.js`文件，上传文件夹即可。无域名可选这种，提供了免费域名
...
const url2 = new URL('https://*.pages.dev/fetch');
...
// 在浏览器、clash客户端需使用这一条
// https://*.pages.dev/fetch?url=http%3A%2F%2Fhttpbin.org%2Fget%3Ftoken%3DXXXXXX%26_ua%3Dclash
...
5. 链接管理器的`拼接`模式内含`fetch`功能(支持`_ua`参数)，同时还含`短链`功能，比`/fetch`更简单直接，强烈推荐使用
...
## 其他问题
1. 如何知道某个`uuid`和`sni`失效了？
	1. 按名称排序，观察某个`uuid`是否全为`-1`或`Timeout`，别名`remark`前段是`uuid`的前4位，可用于区分
	2. 本工具的clash订阅转换功能，轮询是按名称排序的
